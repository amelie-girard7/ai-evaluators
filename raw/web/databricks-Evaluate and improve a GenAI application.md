...
source: https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/evaluate-app
author:
- '[[mssaperla]]'
published: null
created: 2026-05-01
description: Learn how to systematically evaluate new versions of your GenAI application
  in MLflow, assess quality after changes, detect regressions, and iteratively improve
  your app's quality using `mlflow.genai.evaluate()`.
tags:
- clippings
processed: true
...
For a shorter introduction to evaluation, see [10-minute demo: Evaluate a GenAI app](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/getting-started/eval).

This tutorial includes the following steps:

- Create [evaluation datasets](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/eval-datasets) from real usage data.
- Evaluate quality with MLflow's [LLM judges](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/scorers) using the [evaluation harness](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/eval-harness).
...
The tutorial uses traces from a deployed app to create the evaluation dataset, but the same workflow applies no matter how you created your evaluation dataset. For other approaches to creating an evaluation dataset, see [Building MLflow evaluation datasets](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/build-eval-dataset). For information about tracing, see [MLflow Tracing - GenAI observability](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/tracing/).

![Offline monitoring workflow diagram](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/offline-eval.png)
...
1. Install required packages:
	...
	```
2. Create an MLflow experiment. If you are using a Databricks notebook, you can skip this step and use the default notebook experiment. Otherwise, follow the [environment setup quickstart](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/getting-started/connect-environment) to create the experiment and connect to the MLflow Tracking server.
3. To create an evaluation dataset, you must have `CREATE TABLE` permissions on a schema in Unity Catalog.

## Step 1: Create your application

The first step is to create the email generation app. The retrieval component is marked with [`span_type="RETRIEVER"`](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/tracing/span-concepts#retriever-spans) to enable MLflow's retrieval-specific LLM judges.

1. Initialize an OpenAI client to connect to either Databricks-hosted LLMs or LLMs hosted by OpenAI.
	### Databricks-hosted LLMs
	Use `databricks-openai` to get an OpenAI client that connects to Databricks-hosted LLMs. Select a model from the [available foundation models](https://learn.microsoft.com/en-us/azure/databricks/machine-learning/foundation-model-apis/supported-models).
	Python
	```python
	import mlflow
...
![Evaluation app trace](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/new-images/eval-app-guide-initial-trace.gif)
...
In this step, you use MLflow's built-in LLM judges to automatically evaluate different aspects of the GenAI app's quality. To learn more, see [LLM judges](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/scorers) and [code-based scorers](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/custom-scorers).
...
Running [`mlflow.genai.evaluate()`](https://mlflow.org/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate) creates an evaluation run. For details, see [Evaluation runs](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/evaluation-runs).

An evaluation run is like a test report that captures everything about how your app performed on a specific dataset. The evaluation run contains a trace for each row in your evaluation dataset annotated with [feedback](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/human-feedback/dev-annotations) from each judge.
...
3. In the left sidebar, click **Evaluation runs**. The right pane shows a table of traces.
	![Evaluation runs table](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/eval-runs-table.png)
	...
	![Expand table](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/expand-pane.gif)
4. To see the rationale for the **Pass** or **Fail** label, hover over the label.
	![Hover over label to show rationale](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/rationale.gif)
...
1. Click on the request identifier in the **Request** column. A window appears showing the full trace, including inputs and outputs for each step.
	![Request details window](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/request-details.png)
2. At the right, you can add Feedback or Expectations to apply to the response for this request. If you do not see the Assessments pane, click. To add a new Assessment, scroll down and click.
3. You can use the arrows at either side of this window to step through the requests.
	![Step through requests using arrows](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/step-through-requests.png)
...
3. In the left sidebar, click **Evaluation runs**. The left pane shows a a list of evaluation runs for this experiment.
	![Runs pane](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/runs-pane.png)
4. Check the boxes for the runs you want to compare.
5. From the **Actions** drop-down menu, select **Compare**.
	![Select runs to compare](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/select-compare.png)
6. The right pane displays a comparison of each trace in the selected runs.
	![Trace comparison screen](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/run-comparison-table.png)
7. For more details, click on the request identifier in the **Request** column. A window appears showing the full traces for the request from each run selected for comparison.
	![Comparison details window](https://learn.microsoft.com/en-us/azure/databricks/_static/images/mlflow3-genai/eval-monitor/comparison-details.png)
...
[Get notebook](https://docs.databricks.com/notebooks/source/mlflow3/evaluate-improve-genai-app.html)

## Next steps

- [Build evaluation datasets](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/build-eval-dataset) - Prepare data for consistent evaluation runs
- [Evaluate conversations](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/evaluate-conversations) - Assess multi-turn conversations with specialized scorers
- [Conversation simulation](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/conversation-simulation) - Generate synthetic conversations to test your agent with diverse scenarios
- [Create custom LLM judges](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/custom-judge/) - Further customize the LLM judges used in this guide
- [Align judges with human feedback](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/align-judges) - Improve judge accuracy by 30-50% to match your team's standards
- [Create custom code scorers](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/custom-scorers) - Evaluate your app with deterministic code-based scorers
- [Set up production monitoring](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/production-monitoring) - Use the same scorers to monitor quality in production
- [Track app and prompt versions](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/prompt-version-mgmt/prompt-registry/track-prompts-app-versions) - Track application and prompt versions with MLflow.

## Reference guides

- [Evaluation Harness](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/eval-harness) - Comprehensive reference for [`mlflow.genai.evaluate()`](https://mlflow.org/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.evaluate)
- [Scorers](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/scorers) - Deep dive into how scorers assess quality
- [Evaluation Datasets](https://learn.microsoft.com/en-us/azure/databricks/mlflow3/genai/eval-monitor/concepts/eval-datasets) - Learn about versioned datasets for consistent testing
